<template>
	<view class="container">
		...
		<view v-if="isReach">
			<view class="box">
				<view class="cu-bar bg-cyan search">
					<view class="search-form radius">
						...
						<input :value="inputValue" placeholder="输入城市名或拼音" placeholder-style="color:#8E8F97"
							:focus="searchFocus" @input="searchChange" :adjust-position="false" type="text"
							confirm-type="search"></input>
					</view>
					<view class="action" open-type="navigateBack" delta="1" @click="isReach = false">
			...
			<view class="reach-content" v-if="inputValue">
				<block v-show="searchData.length">
					<view class="li borderBottom" v-for="item in searchData" :key="item.cityId"
						@click="citySearchList(item)">{{ item.city }}</view>
				</block>
		...
		<!-- 城市列表 -->
		<view v-show="!isReach">
			<view class="top-search" style="margin-top: 12rpx;">
			...
			<scroll-view class="scroll-view" scroll-y scroll-with-animation="true" enable-back-to-top="true"
				...
				<view class="block">
					...
					<!-- 热门城市 -->
					<view  v-if="false" class="area list-item" id="hot">
						<view style="border-radius: 18px; min-height: 60rpx;" class="cu-bar bg-white">
							<view class="action">
								<text class="cuIcon-title text-blue"></text>热门小区
							</view>
						</view>
						<view class="ul" v-if="hotcites.length">
							<view class="li" hover-class="hover" v-for="item in hotcites" :key="item.cityId"
								@click="selectCity(item)">{{ item.cityName }}</view>
						</view>
					</view>
				</view>
